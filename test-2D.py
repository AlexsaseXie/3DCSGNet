...
for k in data_labels_paths.keys():
    ...
    for batch_idx in range(dataset_sizes[k][1] // config.batch_size):
        ...
        # transfer target expressions into stacks
        target_stacks = parser.expression2stack(target_expressions)
        target_stacks = target_stacks[-1 ,: ,0 ,: ,: ,: ]

        R = np.sum(np.logical_and(stack, target_stacks), (1, 2, 3)) / (np.sum(
            np.logical_or(stack, target_stacks), (1, 2, 3)) + 1)
        Rs += np.sum(R)
    total_iou += Rs
