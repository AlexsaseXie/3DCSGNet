...
class PM_CsgNet(nn.Module):
    ...
    def init_projector(self):
        self.axis = glm.vec3(1,1,1)
        self.transfer_matrix = axis_view_matrix(axis=self.axis)
        self.center = np.dot( self.transfer_matrix,np.array([32,32,32],dtype=float) )
    ...
    def project(self, x):
        ...
        for m in x:
            ...
            point_list = axis_view_place_points(voxel, transfer_matrix = self.transfer_matrix)

            img = z_parrallel_projection_point_simple(point_list,origin_w=128,origin_h=128, origin_z=128, w=128, h=128, center_x=self.center[0], center_y=self.center[1])

            img = np.resize(img,[1,img.shape[0],img.shape[0]])

            new_x.append(img)

        new_x = np.stack(new_x, axis=0)

        # new_x : batch * 1 * 128 * 128
        return new_x
